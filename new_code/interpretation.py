...
# all the other backprop methods call sensitivity_analysis:

def sensitivity_analysis(model, image_tensor, target_class=None, postprocess='abs', apply_sigmoid=True, cuda=False, verbose=False):
    """
    ...
    # Forward pass.
    image_tensor = torch.Tensor(image_tensor)  # convert numpy or list to tensor; (1, 64, 64, 64)
    if cuda:
...
def grad_cam(model, image_tensor, target_class=None, last_conv_layer=None, resize=True, interpolation=1, apply_sigmoid=True, cuda=False, verbose=False):
    """
